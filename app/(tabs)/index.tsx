          ...
          <Card style={styles.stepsCard}>
            <View style={styles.stepsRow}>
              <View>
                ...
                <Text variant="caption">~{stepsToCalories(stepsToday, profile?.bodyweightKg ?? 70)} kcal burned</Text>
              </View>
              <View style={styles.stepsRing}>
                <Text style={styles.stepsRingPct}>{Math.min(Math.round(stepsToday / 10000 * 100), 100)}%</Text>
                <Text variant="caption">of 10k</Text>
              </View>
            </View>
            <View style={styles.stepsTrack}>
              <View style={[styles.stepsFill, { width: `${Math.min(stepsToday / 10000 * 100, 100)}%` as any }]} />
            </View>
          </Card>
        )}
          ...
          <Card accent style={styles.aiCard}>
            <View style={styles.aiHeader}>
              <View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}>
                ...
                <Text variant="label" color={Colors.primary}>COACH AI</Text>
              </View>
            ...
            {aiLoading ? (
              ...
              <Text variant="body" style={{ marginTop: Spacing.sm }}>
                {aiSuggestion ?? (weeklyWorkouts === 0
                  ? 'Complete your first workout to get personalized AI coaching!'
                  : 'Tap to chat with your AI trainer!')}
              </Text>
            )}
          </Card>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  chatLink: { fontSize: FontSize.sm, color: Colors.primary, fontWeight: FontWeight.bold },
  stepsCard: { gap: Spacing.sm },
  stepsRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  stepsValue: { fontSize: FontSize.xxl, fontWeight: FontWeight.heavy, color: Colors.primary },
  stepsRing: { alignItems: 'center' },
  stepsRingPct: { fontSize: FontSize.xl, fontWeight: FontWeight.heavy, color: Colors.success },
  stepsTrack: { height: 6, backgroundColor: Colors.bgCardBorder, borderRadius: Radius.full, overflow: 'hidden' },
  stepsFill: { height: '100%', backgroundColor: Colors.success, borderRadius: Radius.full },
});
